...
Необходимо в папке с файлами создать файл скрипта `directories.sh` и поместит скрипт в него, затем там же создаем файл `list.txt` и помещаем в него список с именами файлов. Имена файлов могут содержать пробелы, прочие допустимые символы и расширения (В нашем случае `.mp4`). Могут и не содержать расширение, но тогда придется проконтролировать не содержится ли в названии файла `точка`, так как оно будет восприниматься скриптом как расщирение файла и все после нее будет удаляться). Каждое название файла должно размещаться на новой строке. Пустых строк в файле не должно быть.
